...
if(isset($_GET['locker_id']))
{
  ...
  $lockerId = base64_decode($_GET['locker_id']);
  $assignId = base64_decode($_GET['assignment']);
  $status = base64_decode($_GET['status']);
  $date = date('Y-m-d H:i:s');
  $KeyReceiveSql = "insert into locker_receive_master(assign_id, receive_date) values('$assignId', '$date')";
  $KeyReceiveRec = mysql_query($KeyReceiveSql);
  if($KeyReceiveRec){

    $updateAssignSql = "update locker_assign set assignment_status = '$status' where assign_id = '$assignId'";
    $updateAssignRec = mysql_query($updateAssignSql);
    if($updateAssignRec)
    {
        $updateLockerSql = "update locker_master set locker_status = 1 where locker_id = '$lockerId'";
        //echo $updateLockerSql;exit;
        $updateLockerRec = mysql_query($updateLockerSql);
        if($updateLockerRec)
        {
            echo "<script>
                alert('Locker Key Received')
            ...
            echo "<script>
                alert('Locker Key Does Not Received')
                location.replace('search_locker.php?');
              </script>";
        }
    }
                        ...
                        <tr>
                           <td width="12%" class="featureHeader" style="padding-top: 2em">
                            ...
                            <td width="10%" class="featureHeader" align="center" style="padding-top: 2em; color: #5D6D7E;">
                              Office</td>
                           <td width="20%" class="featureHeader"><select class="form-control" id="office" name="office" onchange="search_floor()" style="width: 9em;">
                             <option value="0">... Select ...</option>
                             <?php
                                   $office_sql = "select * from office_admin_credentials_master cOffice, office_master officeMaster where cOffice.email_id = '$_SESSION[email_id]' and cOffice.office_id = officeMaster.office_id and officeMaster.office_status = 1";
                                  // echo $floor_sql;exit;
                            ...
                            <td width="12%" class="featureHeader" align="center" style="padding-top: 2em; color: #5D6D7E">
                              Floor Number                           </td>
                           <td width="20%" class="featureHeader"><div id="showFloor"><?php include "search_floor.php";?></div></td>
                            <td width="12%" align="center" class="featureHeader" style="padding-top: 2em;  color: #5D6D7E">
                              Locker Number</td>
                <td width="20%"><input type="txt" style="width: 100%" class="form-control" id="locker_number" name="locker_number" onkeyup="search_locker()">
                </td>
                 <tr>
            <td height="30" align="center" colspan="7"><div id="showList"><?php include "search_locker_list.php";?></div></td>
          </tr>
                        </tr>
                    </table>
                 </div>
