...
if(isset($_POST['Submit']))
{
	...
	$pass_sql = "select * from site_admin_login_master where login_id='$_SESSION[admin_id]'";
	//echo $pass_sql;
	$pass_rec = mysql_query($pass_sql);
	//$pass_row = mysql_num_rows($pass_rec);
	$pass_res = mysql_fetch_assoc($pass_rec);
	//echo "New : ".$pass_res." Old : ".$oldpass;exit;
	if( $pass_res['password'] == $oldpass)
	{
		$update_password = "update site_admin_login_master set password='$new_pass' where login_id='$_SESSION[admin_id]'";
